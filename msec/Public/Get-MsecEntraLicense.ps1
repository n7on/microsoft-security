function Get-MsecEntraLicense {
    ...
    .DESCRIPTION
        ...
        PSCustomObject. This is the tenant's licence inventory: what was bought,
        how much of it is assigned, and which underlying service plans it enables.

        Use this to answer "is this workload even licensed here". That question is
        ...
        all, so an empty CA policy list is expected rather than alarming. The
        ...
        Requires the 'Organization.Read.All' application permission. A clearer
    ...
    .NOTES
        ...
        ServicePlans. A plan can be present but PendingProvisioning or Disabled,
        in which case the capability is not actually usable - so filtering on
        ServicePlans is the honest test. ServicePlanDetail keeps every plan
    ...
    try {
        $skus = @(Invoke-MsecGraphRequest -Path $path -All)
    }
    catch {
        if ($_.Exception.Message -match '403|Forbidden') {
            throw "Forbidden when calling /subscribedSkus. The msec app needs the 'Organization.Read.All' application permission (admin consent required). Re-run New-MsecApp to add and consent it. Original error: $($_.Exception.Message)"
        }
        throw
    }

    # @($null) yields a one-element array containing $null rather than an empty
    # array, which then breaks downstream -contains and .Count checks.
    $arr = { param($v) if ($null -eq $v) { @() } else { @($v) } }

    foreach ($s in $skus) {
        $plans = & $arr $s.servicePlans

        [PSCustomObject]@{
            PSTypeName        = 'MsecEntraLicense'
            ...
            WarningUnits      = [int] $s.prepaidUnits.warning
            SuspendedUnits    = [int] $s.prepaidUnits.suspended
            Assigned          = [int] $s.consumedUnits

            # Usable plans only - see .NOTES on why provisioningStatus matters.
            ServicePlans      = @($plans |
                                    Where-Object { $_.provisioningStatus -eq 'Success' } |
                                    ForEach-Object { $_.servicePlanName })
            ServicePlanDetail = $plans
